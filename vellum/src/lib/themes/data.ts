 ...
 */
import { packThemes } from "./packs";

export type ThemeMode = "light" | "dark";

/** Builtin theme keys (style-pack themes extend the runtime map beyond these). */
...
export interface ThemeFonts {
  heading: string;
  body: string;
  /** Dedicated face for stats/figures/tables (Tome-style numeric slot). */
  numeric?: string;
...
export interface ThemeProperties {
  ...
  /** Brand-kit logo URL — rendered on hero/closing slides when present. */
  brandLogo?: string;
  /**
   * Structural design-token overrides layered over the family preset —
   * lets a theme tune scale/card-policy/density without a new family.
   * Shape: Partial<DesignTokens> from src/lib/design/tokens.ts (typed
   * loosely here to avoid a themes→design import cycle).
   */
  tokens?: Record<string, unknown>;
  /** Chart series palette override (slots beyond accent/primary). */
  chartSeries?: string[];
}
...
const builtinThemes: { [key in ThemeName]: ThemeProperties } = {
  // ==================== DAKTILO / NOIR ====================
  daktilo: {
    name: "Daktilo",
...
/**
 * The full runtime theme registry: 38 builtins + style-pack themes.
 * Pack modules are data-only (type-only imports of ThemeProperties), so
 * there is no import cycle. New themes need no union edit — the map is
 * open-keyed and every consumer resolves dynamically.
 */
